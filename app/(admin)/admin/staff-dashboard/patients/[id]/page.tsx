...
export default function PatientDetailPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      ...
      (async () => {
        setFetching(true);
        try {
          const res = await api.get(`/api/staff/me/patients/${patientId}`);
          setPatient(res.data);
        } catch {
          setNotFound(true);
        } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-12 text-center">
        ...
        <p className="text-slate-500 mt-2 max-w-md font-secondary">
          This patient record doesn&apos;t exist or isn&apos;t linked to any of your appointments.
        </p>
        <Link href="/admin/staff-dashboard/patients" className="mt-4 text-sm font-semibold text-(--primary-plum) hover:underline">
          Back to My Patients
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-10">
      <Link href="/admin/staff-dashboard/patients" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 font-secondary">
